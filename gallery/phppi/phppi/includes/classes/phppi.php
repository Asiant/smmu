...
class PHPPI
{
	...
	function setThemeMode()
	{
		...
		switch ($browser->getBrowser())
		{
			case Browser::BROWSER_ANDROID:
				...
				if ($this->settings['disable_popup_image_viewer_for_mobile'] == true) { $this->settings['use_popup_image_viewer'] = false; }
				break;
	...
	function getDir($dir) {	
		if ($dir_data = $this->getDirData($dir, 'both', true)) {
			$this->vars['folder_list'] = Array();
			$this->vars['file_list'] = Array();
	...
	function prevImageExists($ignore_javascript = false)
	{
		...
		{
			return true;
		} else if ($ignore_javascript == false and $this->settings['use_javascript_navigation'] == true) {
			return true;
		} else {
			return false;
		}
			...
			{	
				if (is_dir($this->vars['dir']['gallery'] . '/' . $request . $dir['dir']))
				{
					if ($this->settings['thumb_folder_show_thumbs'] == true) {
						...
						if ($this->settings['thumb_folder_shuffle'] == true) { shuffle($dir_data['file']); }
				...
				$replace_values = array("?" . $request . $dir['dir'],
					$dir['dir'], 
